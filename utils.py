...
def download_singbox_if_needed():
    ...
    try:
        ...
        if not asset_url:
            print(f"ERROR: Could not find a download link for '{asset_name_keyword}' in the latest release.")
            return
        ...
        with zipfile.ZipFile(io.BytesIO(zip_response.content)) as z:
            ...
            if not exe_path_in_zip:
                print("ERROR: Could not find sing-box.exe inside the downloaded zip file.")
                return
            ...
            if extracted_path != singbox_path:
                ...
                if dir_to_remove and os.path.isdir(dir_to_remove):
                    try:
                        ...
                        print(f"WARNING: Could not remove temporary directory: {dir_to_remove}")
