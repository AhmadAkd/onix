    ...
    def collect_data(self):
        ...
        try:
            ...
            data = {
                ...
                "timestamp": (
                    self.monitor._last_cleanup
                    if hasattr(self.monitor, "_last_cleanup")
                    else 0
                ),
            }
...
def optimize_memory(main_window):
    ...
    try:
        ...
        if result.get("success", False):
            ...
            message = (
                f"Memory optimization failed: {result.get('error', 'Unknown error')}"
            )
...
def force_garbage_collection(main_window):
    ...
    try:
        ...
        main_window.optimization_results.append(
            f"Error forcing garbage collection: {e}"
        )
...
def create_security_tab(main_window):
    ...
    main_window.kill_switch_button.clicked.connect(
        lambda: toggle_kill_switch(main_window)
    )
    ...
    main_window.dns_protection_button.clicked.connect(
        lambda: toggle_dns_protection(main_window)
    )
    ...
    main_window.webrtc_protection_button.clicked.connect(
        lambda: toggle_webrtc_protection(main_window)
    )
    ...
    try:
        if hasattr(main_window, "security_suite"):
            ...
            if status["webrtc_protection"]:
                ...
                main_window.webrtc_protection_button.setText(
                    main_window.tr("Deactivate")
                )
                main_window.webrtc_protection_status.setStyleSheet("color: green;")

    except Exception as e:
        main_window.security_test_results.append(
            f"Error toggling WebRTC protection: {e}"
        )
...
def update_performance_display(main_window, data: Dict[str, Any]):
    ...
    try:
        ...
        if "cpu_usage" in metrics:
            ...
            # Color coding
            if cpu_value > 80:
                main_window.cpu_progress.setStyleSheet(
        ...
        if "memory_usage" in metrics:
            ...
            # Color coding
            if memory_value > 85:
                main_window.memory_progress.setStyleSheet(
                    "QProgressBar::chunk { background-color: #ff4444; }"
                )
        ...
        if status == "good":
            ...
            main_window.performance_status.setText(
                f"⚠️ Performance Warning: {', '.join(issues)}"
            )
            main_window.performance_status.setStyleSheet(
                """
                QLabel {
...
def update_metrics_table(main_window, metrics: Dict[str, Any]):
    ...
    try:
        ...
        for row, (metric_name, metric_data) in enumerate(metrics.items()):
            table.setItem(
                row, 0, QTableWidgetItem(metric_name.replace("_", " ").title())
            )
            table.setItem(
                row, 1, QTableWidgetItem(f"{metric_data.get('current', 0):.2f}")
            )
            table.setItem(
                row, 2, QTableWidgetItem(f"{metric_data.get('average', 0):.2f}")
            )
