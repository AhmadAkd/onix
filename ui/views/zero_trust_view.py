...
class PolicyDialog(QDialog):
    ...
    def load_policy_data(self):
        ...
        self.identity_types_input.setText(",".join(self.policy_data.get("identity_types", [])))
        self.min_trust_combo.setCurrentText(TrustLevel(self.policy_data.get("min_trust_level", 0)).name)
        self.resources_input.setText(",".join(self.policy_data.get("resources", [])))
        self.allowed_ips_input.setText(",".join(self.policy_data.get("allowed_ips", [])))
    ...
    def get_policy_data(self) -> Dict[str, Any]:
        ...
        return {
            "name": self.name_input.text(),
            "identity_types": [t.strip() for t in self.identity_types_input.text().split(",") if t.strip()],
            "min_trust_level": TrustLevel[self.min_trust_combo.currentText()].value,
            "resources": [r.strip() for r in self.resources_input.text().split(",") if r.strip()],
            "allowed_ips": [ip.strip() for ip in self.allowed_ips_input.text().split(",") if ip.strip()],
...
class SecurityChartWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        for i, point in enumerate(self.data_points):
            ...
            y = height - margin - ((point['value'] - min_val) / (max_val - min_val)) * (height - 2 * margin)
            points.append((x, y))
...
def create_access_control_tab(main_window) -> QWidget:
    ...
    log_table.setHorizontalHeaderLabels([
        "Identity", "Resource", "Allowed", "Reason"
    ])
...
def create_security_events_tab(main_window) -> QWidget:
    ...
    type_combo.addItems(["All", "threat_detected", "access_denied", "verification_failed"])
    ...
    refresh_events_button.clicked.connect(lambda: refresh_security_events(main_window, widget))
    ...
    if dialog.exec() == QDialog.Accepted:
        try:
            ...
            if service.register_identity(identity):
                QMessageBox.information(main_window, "Success", f"Identity '{identity.name}' registered successfully!")
                refresh_identities(main_window, widget)
            else:
...
def verify_identity(main_window, widget):
    """تأیید هویت"""
    identity_id, ok = QInputDialog.getText(main_window, "Verify Identity", "Enter Identity ID:")
    if ok and identity_id:
        try:
            ...
            if verified:
                QMessageBox.information(main_window, "Success", f"Identity verified! Trust Level: {trust_level.name}")
            else:
                QMessageBox.warning(main_window, "Failed", "Identity verification failed")
...
def refresh_identities(main_window, widget):
    ...
    try:
        ...
        for i, (identity_id, identity) in enumerate(identities.items()):
            ...
            table.setItem(i, 4, QTableWidgetItem(time.strftime("%Y-%m-%d %H:%M", time.localtime(identity.last_verified))))
...
def add_policy(main_window, widget):
    ...
    if dialog.exec() == QDialog.Accepted:
        try:
            ...
            QMessageBox.information(main_window, "Success", f"Policy '{policy_data['name']}' added successfully!")
            refresh_policies(main_window, widget)
        except Exception as e:
...
def delete_policy(main_window, widget):
    ...
    reply = QMessageBox.question(
        main_window, "Confirm", f"Are you sure you want to delete policy '{policy_name}'?",
...
def refresh_policies(main_window, widget):
    ...
    try:
        ...
        for i, (policy_name, policy) in enumerate(policies.items()):
            ...
            table.setItem(i, 1, QTableWidgetItem(", ".join(policy.get("identity_types", []))))
            table.setItem(i, 2, QTableWidgetItem(TrustLevel(policy.get("min_trust_level", 0)).name))
...
def refresh_access_log(main_window, widget):
    ...
    try:
        ...
        for i, log_entry in enumerate(access_log):
            ...
            table.setItem(i, 2, QTableWidgetItem("Yes" if log_entry["allowed"] else "No"))
...
def refresh_security_events(main_window, widget):
    ...
    try:
        ...
        for i, event in enumerate(events):
            table.setItem(i, 0, QTableWidgetItem(time.strftime("%H:%M:%S", time.localtime(event.timestamp))))
