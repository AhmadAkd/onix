...
class MLChartWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        predicted_values = [p['predicted'] for p in self.prediction_points if 'predicted' in p]
        ...
        self._draw_line(painter, self.data_points, 'actual', min_val, max_val, width, height, margin)
        
        # رسم خط پیش‌بینی‌ها
        if predicted_values:
            painter.setPen(QPen(QColor(239, 68, 68), 2, Qt.DashLine))
            self._draw_line(painter, self.prediction_points, 'predicted', min_val, max_val, width, height, margin)
        
        # رسم نقاط
        painter.setBrush(QBrush(QColor(59, 130, 246)))
        for i, point in enumerate(self.data_points):
            x = margin + (i / (len(self.data_points) - 1)) * (width - 2 * margin)
            y = height - margin - ((point['actual'] - min_val) / (max_val - min_val)) * (height - 2 * margin)
            painter.drawEllipse(x - 3, y - 3, 6, 6)
    
    def _draw_line(self, painter, points, value_key, min_val, max_val, width, height, margin):
        """رسم خط"""
        if len(points) < 2:
            return
        ...
        for i, point in enumerate(points):
            if value_key in point:
                ...
                y = height - margin - ((point[value_key] - min_val) / (max_val - min_val)) * (height - 2 * margin)
                line_points.append((x, y))
        
        for i in range(len(line_points) - 1):
            painter.drawLine(line_points[i][0], line_points[i][1], line_points[i+1][0], line_points[i+1][1])
...
def create_analytics_tab(main_window) -> QWidget:
    ...
    refresh_recommendations_button.clicked.connect(lambda: refresh_recommendations(main_window, widget))
...
def train_new_model(main_window, widget):
    ...
    if dialog.exec() == QDialog.Accepted:
        try:
            ...
            # آموزش مدل
            model = service.performance_predictor.train_model(config['name'], config['type'])
            if model:
                QMessageBox.information(main_window, "Success", f"Model '{model.name}' trained successfully!")
                refresh_models(main_window, widget)
            else:
...
def refresh_models(main_window, widget):
    ...
    try:
        ...
        for i, (name, model) in enumerate(models.items()):
            ...
            table.setItem(i, 4, QTableWidgetItem(time.strftime("%Y-%m-%d %H:%M", time.localtime(model.last_trained))))
...
def delete_model(main_window, widget):
    ...
    reply = QMessageBox.question(
        main_window, "Confirm", f"Are you sure you want to delete model '{model_name}'?",
...
def start_training(main_window, widget):
    ...
    try:
        ...
        if model:
            ...
            widget.log_text.append(f"Model trained: {model.name} (accuracy: {model.accuracy:.3f})")
        else:
...
def refresh_recommendations(main_window, widget):
    ...
    try:
        ...
        for rec in recommendations:
            item_text = f"[{rec['priority'].upper()}] {rec['title']}: {rec['description']}"
