...
class AppLogic:
    ...
    def handle_server_action(self, action, server_data):
        if action == "ping_url":
            self.log(
                self.main_window.tr("Latency testing server: {}").format(
            ...
            reply = QMessageBox.question(
                self.main_window,
                self.main_window.tr("Confirm Deletion"),
                self.main_window.tr(
            ...
            if reply == QMessageBox.Yes:
                ...
                if (
                    self.main_window.selected_config
                    and self.main_window.selected_config.get("id")
                    == server_data.get("id")
                ):
            ...
            if server_link:
                ...
                self.log(
                    self.main_window.tr("Copied link for '{}' to clipboard.").format(
                        server_data.get("name")
                    )
                )
            else:
                self.log(
                    self.main_window.tr("Could not generate a link for '{}'.").format(
                        server_data.get("name")
                    ),
                    LogLevel.WARNING,
                )
            ...
            if server_link:
                ...
                QMessageBox.warning(
                    self.main_window,
                    self.main_window.tr("Error"),
                    self.main_window.tr("Could not generate a link for this server."),
                )
    ...
    def import_from_clipboard(self):
        ...
        for link in links:
            if self.server_manager.add_manual_server(link.strip()):
                added_count += 1
        self.log(
            self.main_window.tr("Imported {} server(s) from clipboard.").format(
                added_count
            )
        )
    ...
    def handle_import_wireguard_config(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self.main_window,
            self.main_window.tr("Import WireGuard Config"),
            "",
            self.main_window.tr("Config Files (*.conf)"),
        )
        if file_path:
            try:
                ...
                self.log(
                    self.main_window.tr(
                        "Failed to read or parse WireGuard file: {}"
                    ).format(e),
                    LogLevel.ERROR,
                )
                QMessageBox.critical(
                    self.main_window,
                    self.main_window.tr("Import Error"),
                    self.main_window.tr(
                        "Could not import the WireGuard file:\n{}"
                    ).format(e),
                )
    ...
    def _scan_and_add_task(self):
        try:
            ...
            if decoded_objects:
                ...
                self.log(
                    self.main_window.tr("No QR code found on the screen."),
                    LogLevel.WARNING,
                )
        finally:
    ...
    def copy_group_links_to_clipboard(self):
        ...
        if self.main_window.server_list_widget.count() == 0:
            self.log(
                self.main_window.tr("No visible servers to copy."), LogLevel.WARNING
            )
            QMessageBox.warning(
                self.main_window,
                self.main_window.tr("Copy Links"),
                self.main_window.tr("There are no servers in the list to copy."),
            )
            return
        visible_servers = []
        ...
        if not visible_servers:
            self.log(
                self.main_window.tr("Could not retrieve server data from the list."),
                LogLevel.ERROR,
            )
            return
        links = [
        ...
        if links:
            ...
            QMessageBox.information(
                self.main_window,
                self.main_window.tr("Copy Successful"),
                self.main_window.tr("Copied {} server link(s) to clipboard.").format(
                    len(links)
                ),
            )
        else:
            QMessageBox.warning(
                self.main_window,
                self.main_window.tr("Copy Failed"),
                self.main_window.tr(
                    "Could not generate links for the visible servers."
                ),
            )
    ...
    def delete_current_group(self):
        ...
        if not current_group:
            self.log(
                self.main_window.tr("No group selected to delete."), LogLevel.WARNING
            )
            return
        reply = QMessageBox.question(
            self.main_window,
            self.main_window.tr("Confirm Group Deletion"),
            self.main_window.tr(
    ...
    def handle_clear_core_log(self):
        ...
        if not os.path.exists(log_file_path):
            QMessageBox.information(
                self.main_window,
                self.main_window.tr("Log File"),
                self.main_window.tr("Core log file does not exist. Nothing to clear."),
            )
            self.log(
                self.main_window.tr("Core log file not found, nothing to clear."),
                LogLevel.INFO,
            )
            return
...
This action cannot be undone."""
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                self.log(
                    self.main_window.tr("Successfully deleted {}").format(
                        log_file_path
                    ),
                    LogLevel.SUCCESS,
                )
                ...
                self.log(
                    self.main_window.tr("Failed to delete log file: {}").format(e),
                    LogLevel.ERROR,
                )
    ...
    def handle_import_profile(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self.main_window,
            self.main_window.tr("Import Profile"),
            "",
            self.main_window.tr("JSON Files (*.json)"),
        )
        if file_path:
            ...
            if success:
                QMessageBox.information(
                    self.main_window,
                    self.main_window.tr("Import Successful"),
                    self.main_window.tr(
    ...
    def handle_export_profile(self):
        file_path, _ = QFileDialog.getSaveFileName(
            self.main_window,
            self.main_window.tr("Export Profile"),
            "onix_profile.json",
            self.main_window.tr("JSON Files (*.json)"),
        )
        if file_path:
            ...
            if success:
                QMessageBox.information(
                    self.main_window,
                    self.main_window.tr("Export Successful"),
                    self.main_window.tr("Profile successfully exported to:\n{}").format(
                        file_path
                    ),
                )
    ...
    def show_subscription_manager(self):
        ...
        dialog = SubscriptionManagerDialog(
            self.main_window, subscriptions=[sub.copy() for sub in current_subs]
        )
        if dialog.exec() == QDialog.Accepted:
            ...
            if updated_subs is not None:
                ...
                reply = QMessageBox.question(
                    self.main_window,
                    self.main_window.tr("Update Subscriptions"),
                    self.main_window.tr(
    ...
    def show_chain_manager(self):
        ...
        dialog = ChainManagerDialog(
            self.main_window,
            chains=[c.copy() for c in current_chains],
        ...
        try:
            ...
            QMessageBox.warning(
                ...
                self.main_window.tr(
                    ...
                    setting_name,
                    (
                        self.main_window.tr("number")
                        if validation_type == "int"
                        else self.main_window.tr("range (e.g., 10-100)")
                    ),
                ),
            )
            widget.setFocus()
    ...
    def save_settings(self):
        ...
        self.settings["connection_mode"] = (
            self.main_window.connection_mode_combo.currentText()
        )
        ...
        self.settings["appearance_mode"] = {
            self.main_window.tr("System"): "System",
            self.main_window.tr("Light"): "Light",
            self.main_window.tr("Dark"): "Dark",
        }.get(self.main_window.appearance_mode_combo.currentText(), "System")
        ...
        self.settings["active_core"] = (
            self.main_window.core_selector_combo.currentText()
        )

        if not self._validate_and_save_setting(
            self.main_window.mux_max_streams_entry,
            "mux_max_streams",
            "Mux Max Streams",
            "int",
        ):
            return
        if not self._validate_and_save_setting(
            self.main_window.tls_fragment_size_entry,
            "tls_fragment_size",
            "TLS Fragment Size",
            "range",
        ):
            return
        if not self._validate_and_save_setting(
            self.main_window.tls_fragment_sleep_entry,
            "tls_fragment_sleep",
            "TLS Fragment Sleep",
            "range",
        ):
            return
        if not self._validate_and_save_setting(
            self.main_window.hysteria_up_speed_entry,
            "hysteria_up_mbps",
            ...
            return
        if not self._validate_and_save_setting(
            self.main_window.hysteria_down_speed_entry,
            "hysteria_down_mbps",
        ...
        self.settings["tls_fragment_enabled"] = (
            self.main_window.tls_fragment_checkbox.isChecked()
        )
        self.settings["mux_enabled"] = self.main_window.mux_enabled_checkbox.isChecked()
        self.settings["mux_protocol"] = (
            self.main_window.mux_protocol_combo.currentText()
        )
