...
class ExamplePlugin(PluginInterface):
    ...
    def initialize(self, app_context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.register_event_handler("server_disconnected", self.on_server_disconnected)
    ...
    def on_server_connected(self, event):
        ...
        if self.log:
            self.log(f"Example plugin: Server connected - {event.data.get('server_name', 'Unknown')}", LogLevel.INFO)
    
    def on_server_disconnected(self, event):
        """Handle server disconnected event."""
        if self.log:
            self.log(f"Example plugin: Server disconnected - {event.data.get('server_name', 'Unknown')}", LogLevel.INFO)
