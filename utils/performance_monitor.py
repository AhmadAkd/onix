...
class PerformanceMonitor:
    ...
    def _check_thresholds(self):
        ...
        try:
            ...
            if self._metrics["cpu_usage"]:
                avg_cpu = sum(self._metrics["cpu_usage"]) / len(
                    self._metrics["cpu_usage"]
                )
            ...
            if self._metrics["memory_usage"]:
                avg_memory = sum(self._metrics["memory_usage"]) / len(
            ...
            if self._detect_performance_degradation():
                self._trigger_callbacks(
                    "performance_degraded", self._get_current_metrics()
                )
    ...
    def _perform_cleanup(self):
        ...
        try:
            ...
            if collected > 0:
                self.log(
                    f"Garbage collection freed {collected} objects", LogLevel.DEBUG
                )
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {"status": "good", "issues": [], "recommendations": []}

            # Check CPU
            if (
                metrics.get("cpu_usage", {}).get("average", 0)
            ...
            # Check Memory
            if (
                metrics.get("memory_usage", {}).get("average", 0)
                > self._thresholds["memory_high"]
            ):
                summary["status"] = "warning"
                summary["issues"].append("High memory usage")
                summary["recommendations"].append(
                    "Restart application or close other programs"
                )
            ...
            return {
                "status": "error",
                "issues": ["Unable to get performance data"],
                "recommendations": [],
            }
...
class PerformanceOptimizer:
    ...
    def optimize_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.log(
                f"Memory optimization freed {result['memory_freed_mb']:.2f} MB",
                LogLevel.INFO,
            )
            return result
    ...
    def get_optimization_recommendations(self) -> List[str]:
        ...
        try:
            ...
            if cpu_percent > 80:
                recommendations.append(
                    "High CPU usage detected - consider reducing application load"
                )
            ...
            if disk.percent > 90:
                recommendations.append(
                    "Low disk space detected - consider freeing up space"
                )
