...
class TestServerManager(unittest.TestCase):
    ...
    def test_load_servers_with_migration(self):
        ...
        old_settings = {
            "servers": {
                "no-id-group": [{"name": "server_no_id", "group": "no-id-group"}]
            }
        }
        with patch("managers.server_manager.settings_manager.save_settings"):
            ...
            manager._save_timer = (
                MagicMock()
            )  # Manually mock the timer for this instance
    ...
    def test_add_duplicate_server(self):
        ...
        with patch(
            "managers.server_manager.link_parser.parse_server_link"
        ) as mock_parse:
            mock_parse.return_value = {"name": "duplicate", "server": "a", "port": 1}
    ...
    def test_delete_server(self):
        ...
        server_to_delete = self.server_manager.get_servers_by_group("group1")[0]
    ...
    def test_sort_servers_by_ping(self):
        ...
        sorted_names = [
            s["name"] for s in self.server_manager.get_servers_by_group("group1")
        ]
    ...
    def test_fetch_and_process_subscription_success(self, mock_get):
        ...
        with patch.object(self.server_manager, "add_manual_server") as mock_add:
            ...
            mock_get.assert_called_once_with("http://example.com/sub", timeout=15)
            mock_add.assert_called_once_with(
                "vless://uuid@server42:443#test1", group_name="TestSub", update_ui=False
            )
    ...
    def test_fetch_and_process_subscription_network_error(self, mock_get):
        ...
        added_count, error = self.server_manager._fetch_and_process_subscription(sub)

        self.assertEqual(added_count, 0)
        self.assertIsInstance(error, requests.exceptions.RequestException)
        self.callbacks["log"].assert_any_call(
            "Failed to update subscription 'TestSub': Network Down", LogLevel.ERROR
        )
    ...
    def test_update_subscriptions_task_orchestration(self, mock_executor_cls):
        ...
        subs = [{"name": "Sub1", "url": "url1"}, {"name": "Sub2", "url": "url2"}]

        # Mock the worker function to return predictable results
        with patch.object(
            self.server_manager, "_fetch_and_process_subscription"
        ) as mock_fetch:
            ...
            mock_fetch.assert_has_calls([call(subs[0]), call(subs[1])], any_order=True)
