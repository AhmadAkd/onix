...
class ExportService:
    ...
    @staticmethod
    def export_health_stats_to_csv(
        ...
        with open(filepath, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for server in servers:
                ...
                if last_test > 0:
                    last_test_str = datetime.fromtimestamp(last_test).strftime(
                        "%Y-%m-%d %H:%M:%S"
                    )
                else:
                    last_test_str = "Never"
                ...
                writer.writerow(
                    {
                        ...
                        "TCP EMA (ms)": (
                            f"{stats.get('tcp_ema', 0):.1f}"
                            if stats.get("tcp_ema")
                            else "N/A"
                        ),
                        "URL EMA (ms)": (
                            f"{stats.get('url_ema', 0):.1f}"
                            if stats.get("url_ema")
                            else "N/A"
                        ),
                        "Failures": failures,
    ...
    @staticmethod
    def export_server_list_to_csv(
        servers: List[Dict[str, Any]], filename: str = None
    ) -> str:
